...
/*
check services for Virtual Machine
*/
BOOL VMDriverServices()
{
	...
	SC_HANDLE hSCM = OpenSCManager(NULL, SERVICES_ACTIVE_DATABASE, SC_MANAGER_CONNECT | SC_MANAGER_ENUMERATE_SERVICE); //open the service control manager
	if (hSCM != NULL)
	{
		...
		if (get_services(hSCM, SERVICE_DRIVER, &services, &serviceCount))
		{
			...
			for (DWORD i = 0; i < serviceCount; i++)
			{
				...
				{
					if (StrCmpIW(services[i].lpServiceName, KnownVMServices[s]) == 0) ok = false; //vm detected
				}
			}
			if (ok) return FALSE;
			free(services);
		}
		else printf("Failed to get services list.\n");
	}
	else printf("Failed tp get SCM handle.\n");
	return TRUE;
}
...
BOOL get_services(_In_ SC_HANDLE hServiceManager, _In_ DWORD serviceType, _Out_ ENUM_SERVICE_STATUS_PROCESS** servicesBuffer, _Out_ DWORD* serviceCount)
{
	...
	{
		//success and wo enumerated all the services
